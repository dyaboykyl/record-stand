...
// Borrowed from
// https://github.com/atomic14/esp32_sdcard_audio/blob/main/arduino-wav-sdcard/lib/wav_file/src/WAVFileWriter.cpp
#pragma once
#pragma pack(push, 1)
typedef struct _wav_header {
  ...
  int wav_size =
      0;  // Size of the wav portion of the file, which follows the first 8 bytes. File size - 8
  ...
  int sample_rate = 0;  // TBD after recording
  ...
  int data_bytes =
      0;  // Number of bytes in data. Number of samples * num_channels * sample byte size
